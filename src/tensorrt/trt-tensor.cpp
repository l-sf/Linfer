...
namespace TRT{
    ...
    static int check_trans_device_id(int device_id){
        if(device_id != CURRENT_DEVICE_ID){
            CUDATools::check_device_id(device_id);
            return device_id;
        }
    ...
    MixMemory::MixMemory(int device_id) {
        device_id_ = check_trans_device_id(device_id);
    }
    ...
    Tensor::Tensor(TRT::DataType dtype, std::shared_ptr<MixMemory> data, int device_id) {
        dtype_ = dtype;
    ...
    Tensor::Tensor(int n, int c, int h, int w, TRT::DataType dtpye, std::shared_ptr<MixMemory> data, int device_id) {
        dtype_ = dtpye;
        device_id_ = get_device(device_id);
        descriptor_string_[0] = 0;
        setup_data(data);
    ...
    Tensor::Tensor(int ndims, const int *dims, TRT::DataType dtype, std::shared_ptr<MixMemory> data, int device_id) {
        dtype_ = dtype;
        device_id_ = get_device(device_id);
        descriptor_string_[0] = 0;
        setup_data(data);
    ...
    Tensor::Tensor(const std::vector<int> &dims, TRT::DataType dtype, std::shared_ptr<MixMemory> data, int device_id) {
        dtype_ = dtype;
        device_id_ = get_device(device_id);
        descriptor_string_[0] = 0;
        setup_data(data);
    ...
    Tensor& Tensor::resize(int ndims, const int* dims) {
        ...
        strides_.resize(setup_dims.size());
    ...
    int Tensor::numel(int start_axis) const {
        int size = shape_.empty() ? 0 : 1;
    ...
    Tensor& Tensor::release() {
        ...
        bytes_ = 0;
        head_ = DataHead::Init;
        if(owner_stream_ && stream_ != nullptr){
            CUDATools::AutoDevice auto_device_exchange(device_id_);
    ...
    Tensor& Tensor::copy_from_gpu(size_t offset, const void *src, size_t num_element, int device_id) {
        ...
        size_t copied_bytes = num_element * element_size();
        ...
        if(head_ == DataHead::Device){
            ...
            if(current_device_id != device_id_)
                checkCudaRuntime(cudaMemcpyPeerAsync(gpu<unsigned char>() + offset_location, device_id_, src, current_device_id, copied_bytes, stream_));
            else
    ...
    shared_ptr<Tensor> Tensor::clone() const {
        ...
        if(head_ == DataHead::Init)
            return new_tensor;
        if(head_ == DataHead::Host)
            memcpy(new_tensor->cpu(), data_->cpu(), bytes_);
        else if(head_ == DataHead::Device){
            CUDATools::AutoDevice auto_device_exchange(device_id_);
            checkCudaRuntime(cudaMemcpyAsync(new_tensor->gpu(), data_->gpu(), bytes_, cudaMemcpyDeviceToDevice, stream_));
        }
        return new_tensor;
    }
    ...
    void Tensor::reference_data(const std::vector<int> &shape, void *cpu, size_t cpu_size, void *gpu, size_t gpu_size,
                                TRT::DataType dtype) {
        dtype_ = dtype;
        data_->reference_data(cpu, cpu_size, gpu, gpu_size);
        setup_data(data_);
        resize(shape);
    }
    ...
    Tensor& Tensor::set_mat(int n, const cv::Mat& image) {
        assert(!image.empty() && CV_MAT_DEPTH(image.type()) == CV_32F && dtype_ == DataType::Float);
    ...
    Tensor& Tensor::set_norm_mat(int n, const cv::Mat &image, float *mean, float *std) {
        assert(!image.empty() && image.channels() == 3 && dtype_ == DataType::Float);
        ...
        if(image.size() != cv::Size(width, height))
            cv::resize(image, image, cv::Size(width, height));
        if(CV_MAT_DEPTH(image.type()) != CV_32F)
    ...
    cv::Mat Tensor::at_mat(int n, int c) {
        return {height(), width(), CV_32F, cpu<float>(n, c)};
    }
    ...
    bool Tensor::save_to_file(const std::string& file) const{
        ...
        fwrite(((Tensor*)this)->cpu(), 1, bytes_, f);
        fclose(f);
        return true;
    }
