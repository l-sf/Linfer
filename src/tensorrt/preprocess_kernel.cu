...
namespace CUDAKernel {
    ...
    Norm Norm::mean_std(const float *mean, const float *std, float alpha, CUDAKernel::ChannelType channel_type) {
        Norm out;
    ...
    Norm Norm::alpha_beta(float alpha, float beta, CUDAKernel::ChannelType channel_type) {
        Norm out;
    ...
    template<class T>
    static __inline__ __device__ T limit(T value, T low, T high){
            ...
            float sx, float sy, Norm norm, int edge)
    {
        ...
        float lx = rint((src_x - x_low) * INTER_RESIZE_COEF_SCALE);;
        float ly = rint((src_y - y_low) * INTER_RESIZE_COEF_SCALE);
        float hx = INTER_RESIZE_COEF_SCALE - lx;
        float hy = INTER_RESIZE_COEF_SCALE - ly;
        float w1 = hx * hy;
        float w2 = lx * hy;
        float w3 = hx * ly;
        float w4 = lx * ly;
        ...
        uint8_t* v3 = v3 = src + y_high * src_line_size + x_low * 3;
        uint8_t* v4 = v4 = src + y_high * src_line_size + x_high * 3;
    ...
    __global__ void warp_affine_bilinear_and_normalize_plane_kernel(
            uint8_t* src, int src_line_size, int src_width, int src_height,
            float* dst, int dst_width, int dst_height,
            uint8_t const_value, float* matrix_2x3,
            Norm norm, int edge)
    {
        ...
        int dx = position % dst_width;
        int dy = position / dst_width;
        float src_x = matrix_2x3[0] * dx + matrix_2x3[1] * dy + matrix_2x3[2];
        float src_y = matrix_2x3[3] * dx + matrix_2x3[4] * dy + matrix_2x3[5];
        float c0, c1, c2;
        ...
        if(src_x <= -1 || src_x >= src_width || src_y <= -1 || src_y >= src_height){
            ...
            uint8_t const_values[] = {const_value, const_value, const_value};
    ...
    __global__ void warp_affine_bilinear_and_normalize_focus_kernel(
            uint8_t* src, int src_line_size, int src_width, int src_height,
            float* dst, int dst_width, int dst_height,
            uint8_t const_value, float* matrix_1x3,
            Norm norm, int edge)
    {
        int position = blockDim.x * blockIdx.x + threadIdx.x;
        if(position >= edge) return;
        ...
        int dx = position % dst_width;
        int dy = position / dst_width;
        float src_x = matrix_1x3[0] * dx + matrix_1x3[1];
        float src_y = matrix_1x3[0] * dx + matrix_1x3[2];
        float c0, c1, c2;
    ...
    __global__ void normalize_feature_kernel(
            float* feature_array, int num_feature, int feature_length, int edge)
    {
        /*
		&   1 gz         bi.z   0
		*   1 gy         bi.y   0
    ...
    static __global__ void convert_nv12_to_bgr_kernel(
            ...
            int linesize, uint8_t* dst_bgr, int edge)
    {
        int position = blockDim.x * blockIdx.x + threadIdx.x;
        if (position >= edge) return;
    ...
    void warp_affine_bilinear_and_normalize_plane(
            ...
            cudaStream_t stream)
    {
        int numjobs = dst_width * dst_height;
        dim3 grids = CUDATools::grid_dims(numjobs);
        dim3 blocks = CUDATools::block_dims(numjobs);

        checkCudaKernel(warp_affine_bilinear_and_normalize_plane_kernel<<<grids, blocks, 0, stream>>>(
                src, src_line_size, src_width, src_height,
                dst, dst_width, dst_height,
                const_value, matrix_2x3, norm, numjobs
                ));
    }
    ...
    void warp_affine_bilinear_and_normalize_focus(
            uint8_t* src, int src_line_size, int src_width, int src_height,
            float* dst, int dst_width, int dst_height,
            float* matrix_1x3, uint8_t const_value, const Norm& norm,
            cudaStream_t stream)
    {
        int numjobs = dst_width * dst_height;
        dim3 grids = CUDATools::grid_dims(numjobs);
        ...
        checkCudaKernel(warp_affine_bilinear_and_normalize_focus_kernel<<<grids, blocks, 0, stream>>>(
                src, src_line_size, src_width, src_height,
                dst, dst_width, dst_height,
                const_value, matrix_1x3, norm, numjobs
        ));
    }
    ...
    {
        int numjobs   = dst_width * dst_height;
        auto grid  = CUDATools::grid_dims(numjobs);
        auto block = CUDATools::block_dims(numjobs);

        checkCudaKernel(resize_bilinear_and_normalize_kernel <<<grid, block, 0, stream >>> (
                src, src_line_size,
                src_width, src_height, dst,
                dst_width, dst_height, src_width/(float)dst_width, src_height/(float)dst_height, norm, numjobs
        ));
    }

    void convert_nv12_to_bgr_invoke(
            const uint8_t* y, const uint8_t* uv, int width, int height, 
            int linesize, uint8_t* dst, cudaStream_t stream)
    {
